...
def extract_lowest_price(text: str) -> int | None:
    ...
    """
    # Prijspatroon: 3-7 cijfers (met punten als duizendscheider) gevolgd door '€'
    ...
    for line in text.splitlines():
        ...
        for match in PRICE_RE.finditer(line):
            ...
            num = int(raw)
            # Sanity-check: realistische woningprijs
...
def find_pricelist_pdf(dbx: dropbox.Dropbox, project_path: str) -> str | None:
    ...
    for folder_name in candidate_folders:
        ...
        try:
            ...
            continue
        # Verzamel PDFs die NIET in een 'dated'-submap zitten
        pdfs: list[tuple[str, str]] = []  # (server_modified, path)
...
def list_subfolders(dbx: dropbox.Dropbox, path: str) -> list[tuple[str, str]]:
    """Geeft [(name, path_display)] terug voor alle submappen."""
    folders = []
    try:
...
def sync(dbx: dropbox.Dropbox, projects: dict) -> dict:
    """
    Doorloopt alle regio's → ontwikkelaars → projecten.
    Koppelt elke projectmap aan een slug in projects{}.
    Geeft een dict terug: slug → nieuwe_prijs (alleen wijzigingen).
    """
    # Bouw een slug-lookup: normalize(projectnaam) → slug
    slug_by_norm: dict[str, str] = {}
    for slug in projects:
        # Gebruik de NL-naam als primaire match
        try:
            nl_name = projects[slug]["nl"]["name"]
            slug_by_norm[normalize(nl_name)] = slug
        except (KeyError, TypeError):
            pass
        # Slug zelf ook als fallback
    ...
    for region_path in REGION_PATHS:
        ...
        for dev_name, dev_path in sorted(developers):
            ...
            if not project_folders:
                # Sommige mappen zijn direct projectmappen (geen ontwikkelaarslaag)
            ...
            for proj_name, proj_path in project_folders:
                ...
                if not slug:
                    # Probeer ook zonder "residences", "homes", "properties"
                ...
                if not slug:
                    # Geen match → overslaan (project bestaat niet op de site)
                    continue
...
def apply_changes(projects: dict, changes: dict[str, int]) -> dict:
    """Past de prijzen toe in de projects-dict."""
